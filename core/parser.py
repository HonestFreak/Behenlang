...
# Parser is defined using SLY from Parser class
...
class parser(Parser):
    ...
    @_('BEHEN PRINT LPAREN params RPAREN')
    def statement(self,p):
        return ('FuncCall',{'params':p.params,'name':'print'})
        
    @_('DEF KEYWORDS KEYWORDS LPAREN def_params RPAREN  LBRACE statements RBRACE')
    def statement(self,p):
        return ('Def',{'name':p.KEYWORDS1,'return':p.KEYWORDS0,'body':p.statements, 'def_params':p.def_params if p.def_params else []})

    @_('BEHEN IF expr LBRACE statements RBRACE')
    def statement(self,p):
        return ('If',{'body':p.statements,'test':p.expr,'orelse':[]})

    @_('BEHEN IF expr LBRACE statements RBRACE ELSE LBRACE statements RBRACE')
    def statement(self,p):
        return ('If',{'body':p.statements0,'test':p.expr,'orelse':p.statements1})


    @_('BEHEN WHILE expr LBRACE statements RBRACE')
    def statement(self,p):
    ...
    def statement(self,p):
        return('VarAssign',{'value':p.expr,'name':p.KEYWORDS})
        
    
    @_('KEYWORDS LPAREN params RPAREN')
    def statement(self,p):
        return ('FuncCall',{'params':p.params,'name':p.KEYWORDS})
    ...
    @_('NUMBER')
    def expr(self,p):
        return('Number',{'value':p.NUMBER})
    
    @_('MINUS NUMBER')
    def expr(self,p):
        return('Number',{'value':int(p.NUMBER)*-1})
